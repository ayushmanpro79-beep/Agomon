...
const normalizeItem = (it: any) => (typeof it === 'string' ? { image: it, alt: '' } : it);

type DepthCarouselProps = {
  items?: Array<string | { image: string; alt?: string; id?: string }>;
  cardWidth?: number;
  cardHeight?: number;
  radius?: number;
  tint?: string;
  ...
  onCardClick?: (index: number, item: any) => void;
  className?: string;
};
...
const DepthCarousel = ({
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      e.preventDefault();
      tweenRef.current?.kill();
      const raw = Math.abs((e as any).deltaX) > Math.abs(e.deltaY) ? (e as any).deltaX : e.deltaY;
      const delta = (e as any).deltaMode === 1 ? raw * 24 : raw;
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (!drag.moved && Math.abs(dx) > 4) {
        drag.moved = true;
        (rootRef.current as any)?.setPointerCapture?.(drag.id);
      }
  ...
  const handleCardClick = useCallback(
    (index: number) => {
      ...
      // if clicking non-active, just focus; if active, trigger expanded view
      if (index !== focusRef.current) {
        setFocus(index, true);
      } else {
        onCardClickProp?.(index, data[index]);
      }
    },
    [setFocus, data, onCardClickProp],
  );
